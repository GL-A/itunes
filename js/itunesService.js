angular.module('itunes').service('itunesService', function($http, $q){
  ...
  //You can return the http request or you can make your own promise in order to manipulate the data before you resolve it.
    ...
    return $http({
      ...
      var artistArr = [];
      var artistCheck = response.data.results;
      for(var i = 0;  i < artistCheck.length; i++){
        artistArr.push({
          AlbumArt: artistCheck[i].artworkUrl100,
          Artist: artistCheck[i].artistName,
          Collection: artistCheck[i].collectionName,
          CollectionPrice: artistCheck[i].collectionPrice,
          Play: artistCheck[i].previewUrl,
          Type: artistCheck[i].kind
        })
      }
      return artistArr;
      })
